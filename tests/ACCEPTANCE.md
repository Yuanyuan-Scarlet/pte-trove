...
- 生成阶段约 42 条彩带全部从全屏底部正中心同一原点持续错峰向上扇形喷射，再向左右和上方散开；同一轮中约 36 条正常力度彩带覆盖上半屏，另有约 6 条较小力度彩带只到达中部或中下部；每轮约 3.7 至 4.7 秒。彩带到达各自高点后保持完全不透明至动画进度 67%，随后保持角度和缩放，沿外侧约 4vw、下方约 6vh 的连续轨迹移动，并用原淡出时长的 60% 线性、连续地从透明度 1 渐变到 0，无透明度跳变或抖动；生成完成后再播放一次收尾动效。
- 彩带包含红、橙、黄、黄绿、绿、青、蓝、靛、紫和粉等至少 12 种颜色，并混合边缘整齐的直条与自然弧形两类形态；两类彩带的宽度、质感和阴影统一，无快速摆动或闪烁；不显示礼花筒图标、文字气泡或文字框。
- 彩带层位于全部页面模块之上且不接收指针事件；进度、状态文字和操作元素清晰并保持可点击，WFD 页面仍保持原有蓝色主题。
- 生成状态持续期间彩带保持循环；状态切换为可下载后，下载界面与彩带同时显示并继续循环，刷新已完成的下载页面后彩带仍持续喷射。
- 手机端减少彩带数量；“减少动态效果”模式下不显示庆祝装饰。
- PDF 可打开且保持 302 页，每页具有“祝考试好运 UPUP”、完整手机号、中央浅色 Logo 和右下角浅色 Logo。
...
- ZIP 内五个 PDF 均具有前置 2 个空格的“祝考试好运 UPUP”、前置 4 个空格的完整手机号、中央 Logo 和右下角 Logo；全部文字与 Logo 水印的透明度均为 1.5%。
