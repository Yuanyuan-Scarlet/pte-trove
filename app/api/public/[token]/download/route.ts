import { errorResponse, HttpError, requestPublicOrigin } from "@/lib/http";
...
export function unauthorizedDownloadResponse(request: Request, token: string): Response {
  if (isBrowserNavigation(request)) {
    return Response.redirect(`${requestPublicOrigin(request)}/g/${encodeURIComponent(token)}#auth-required`, 303);
  }
  return errorResponse(new HttpError(401, "请先验证手机号和订单号", "BUYER_AUTH_REQUIRED"));
}
...
export async function GET(request: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (!access) return unauthorizedDownloadResponse(request, token);
    const object = await openStorageObject(access.storageKey);
